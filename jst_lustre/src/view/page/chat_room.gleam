...
pub fn view(
  ...
  let name_prompt = case user_name {
    None -> [
      html.div([attr.class("mb-4 p-4 bg-yellow-100 rounded border border-yellow-300")], [
        html.p([attr.class("mb-2 text-yellow-800 font-medium")], [
          html.text("Enter your name to join the chat:")
        ]),
        html.input([
          attr.class("w-full p-2 border border-yellow-300 rounded focus:outline-none focus:ring-2 focus:ring-yellow-500"),
          attr.placeholder("Your name"),
          attr.id("user-name-input"),
        ]),
        html.button(
          [
            attr.class("mt-2 bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded transition-colors"),
            mouse.on_mouse_down_no_right(on_set_name("")), // Will be updated with actual input value
          ],
          [html.text("Join Chat")]
        ),
      ])
    ...
    Some(name) -> [
      html.div([attr.class("mb-4 p-2 bg-green-100 rounded border border-green-300")], [
